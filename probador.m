...
sum_gammas=pi*1*norm(u_inf)*(0.5+alphad*(pi/180));
...
coord_vor_global=local_a_global(coord_control,x_oo,z_oo,theta_inst);
...
elseif caso==2 %semicirculo de joukowski estacionario
    Cl_jou=2*pi*(0.5+alphad*(pi/180));
    L_teorica=0.5*rho*norm(u_inf)^2*1*Cl_jou;%cuerda=1
    ratio=(Cl_jou-Cl_total(numel(Cl_total)-1))/Cl_jou;error=ratio*100
